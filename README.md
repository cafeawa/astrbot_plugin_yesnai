...
| `confirm_paid_requests` | 非免费请求是否先查询报价并等待 y/n 确认，默认关闭 |
| `suppress_image_llm` | 拦截含图片的消息触发默认 LLM 对话，默认关闭 |
...
```
/ynai i2i 改成油画风格 --strength 0.5
/ynai ref 保持这个角色，换一个姿势 --strength 1.0
```
...
> - 开启（NSFW 模式）：不自动注入任何内容，完全由用户/LLM 提供的 tag 决定

### 其他

```
/ynai model
/ynai help    # 配置了 t2i_url 时渲染为文转图帮助卡片，否则返回文本
```
